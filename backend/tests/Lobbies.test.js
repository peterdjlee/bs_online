    ...
    lobby_codes.forEach(code => {
        for (let i = 0; i < 10; i+= 1) {
            ...
            if (result.passed) {
                ...
                expect(lobbies.getLobbyOf(socket_id).data).toStrictEqual(code);
            }
            else
...
test("connection record / records the correct number of sockets", () => {
    ...
    expect(lobbies.totalPlayers().data).toStrictEqual(0);
    ...
    expect(lobbies.totalPlayers().data).toStrictEqual(total);
});
